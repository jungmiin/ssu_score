...
{
	FILE *fp;
	char tmp[BUFLEN], tmp2[BUFLEN], tmp3[BUFLEN];
	char *quizBuf[QNUM];
	char *quizName[QNUM];
	char *iStudent[FILELEN];
	char iStudentPrint[QNUM][FILELEN];
	int i = 0, q = 0, comma = 0, k=0;
	char *p, *p2, *p3;

	if((fp = fopen("score.csv", "r")) == NULL){ //score.csv 파일 읽음
		fprintf(stderr, "file open error for score.csv\n");
		return;
	}
	...
	p=strtok(tmp,","); //맨 처음 ','가 나오기 전까지 자름 

	k=0;

	while(p!=NULL){ //score.csv 파일의 첫 번째 행을 quizName에 집어넣기
		quizBuf[i+1]=p;
		quizName[i+1]=p;
		q++;
		i++;
	...
	memcpy(quizName, quizBuf, sizeof(quizBuf));

	while(fscanf(fp, "%s\n", tmp3) != EOF) //score.csv 파일의 두 번째 행부터 읽음
	{
		strcpy(tmp2,tmp3);

		p3=strtok(tmp3,",");
		if(!is_exist(ids, tmp3))//입력받은 학번과 맞는지 검사
			continue;
		i=0;
		p2=strtok(tmp2,",");
		while(p2!=NULL){ //iStudent에 해당 학번의 채점 리스트 저장	
			iStudent[i]=p2;
			i++;
			p2 = strtok(NULL,",");
		}
		...
		strcpy(iStudentPrint[k],iStudent[0]);
		k++;

	        printf("%s's wrong answer :\n", iStudent[0]);
			for(int j=1;j<q;j++){ //0점인 문제를 찾아서 출력
				if(!strcmp(iStudent[j],"0")){
				if(comma==0){
					printf("%s", quizName[j]);
					comma=1;
				}
	...
	for(i=0;strcmp(ids[i],"")!=0;i++){
		cnt=0;
		for(z=0;strcmp(iStudentPrint[z],"")!=0;z++){
			if(!strcmp(iStudentPrint[z], ids[i])){
				break;
			}
			else{
				cnt++;
...
void read_scoreTable(char *path)
{
	...
	char qname[FILELEN];
	char score[BUFLEN];
	char modifyqname[FILELEN];
	char *qname_cut;
	char qname_cut_tmp[BUFLEN];
	char tmp[BUFLEN];
	double modscore;
	int idx = 0;
	int i=0;
	...
	if(mOption){ //추가 구현
		while(1){
			...
			scanf("%s", modifyqname);
			if(!strcmp(modifyqname,"no"))
				break;
			for(i=0; i<idx; i++){
				strcpy(qname_cut_tmp,score_table[i].qname);
				qname_cut=strtok(qname_cut_tmp, ".");
				if(!strcmp(modifyqname,qname_cut)){
			        printf("Current score : %.1f\nNew score : ", score_table[i].score);
					scanf("%lf",&modscore);
					score_table[i].score=modscore;
					break;
			    }
			}
			if(i==idx)
...
void make_scoreTable(char *ansDir)
{
	...
	if((dp = opendir(ansDir)) == NULL){
		fprintf(stderr, "open dir error for first %s\n", ansDir);
		return;
	}	
...
//			fprintf(stderr, "open dir error for second %s\n", tmp);
//			return;
//		}
